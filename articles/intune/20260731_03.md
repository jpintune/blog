...
現在、Android を Intune で MDM 管理する場合、原則として **Android Enterprise** に対応したデバイスであることが前提となります。従来の Device Administrator（デバイス管理者）方式は Google により非推奨・機能縮小が進んでおり、新規の管理方式としては推奨されません。
...
Android は Google が提供する OS をベースに、**各デバイス メーカーが独自のカスタマイズを加えて出荷**しています。このため、同じ Android バージョン・同じ Intune ポリシーであっても、**メーカーによって MDM 制御の挙動が異なる**場合があります。

たとえば、以下のような差異が発生することがあります。

- 電源管理・バッテリー最適化の独自実装により、Intune／Company Portal アプリのバックグラウンド動作が制限される
- 特定の制限ポリシーやパスワード ポリシーの適用結果がメーカー実装に依存する
- OEM 独自のセキュリティ機能が MDM 制御と競合する

これらは Intune 側の設定では解消できない、**デバイス メーカー側の仕様に起因する事象**である可能性があります。そのため、特定メーカー・特定モデルでのみ発生する事象については、**デバイス メーカー（ベンダー）へのご確認が必要**となる場合があります。

お問い合わせの際は、**メーカー名・モデル名・Android バージョン**をあわせてお知らせいただくと、この切り分けを迅速に行えます。

---

## 3. Intune は「アプリ／設定を直接配布」しているわけではありません（GMS 経由）

Android では、Intune から**直接的に**アプリや設定をデバイスへ配布しているわけではなく、**GMS（Google Mobile Services）を経由して**処理が行われる仕組みになっています。
...
![Intune から Android へアプリ／設定が届くまでの流れ（GMS 経由）](2026073103/gms-flow.png)
...
これは異常ではなく、Android のアーキテクチャ上、**想定される動作**です。ネットワーク状況、デバイスの電源状態、Google Play サービスの同期タイミングなどにより、反映までの時間は変動します。

配布遅延の際のポイントとして、遅延は **Intune → GMS → Android Device Policy App** のいずれの処理段階でも発生する可能性があります。特に**初回登録時**は、デバイスに割り当てられたすべてのポリシーやアプリが **Intune → GMS → Android Device Policy App** の経路でまとめて処理されるため、適用や配布に時間を要する場合があります。

そのため、適用直後に反映が確認できない場合でも、しばらく時間をおいてから再度ご確認いただくことをおすすめします。「Intune 上は成功しているのに反映されない」という事象は、この GMS 経由の遅延が原因であるケースが少なくありません。
